...
class BreakthroughManager:
    ...
    def __init__(self, db: DataBase, config_manager: ConfigManager, config: dict):
        ...
        self.config = config
        # 死亡结算统一由 DeathManager 负责（与世界事件共享同一套规则）
        self.death_manager = DeathManager(db, config_manager, config)
    ...
    async def execute_breakthrough(
        ...
        if breakthrough_success:
            ...
            if died:
                ...
                # 死亡机制（批次1）：统一交给 DeathManager，保证与世界事件等系统规则一致
                outcome = await self.death_manager.apply_death(player)

                if outcome.is_rebirth:
                    current_level_name = level_data[player.level_index]["level_name"]
                    old_level_name = level_data[outcome.old_level_index]["level_name"]
                    level_change_text = (
                    ...
                    death_msg = (
                        ...
                        f"🌟 天道垂怜，获得【劫后重生】机会\n"
                        f"📉 {level_change_text}\n"
                        f"💫 保留 {outcome.keep_rate:.0%} 修为（当前修为：{player.experience}）\n"
                        f"━━━━━━━━━━━━━━━\n"
                ...
                death_msg = (
                    ...
                    f"📊 修为锁定 50%，每小时流失 {outcome.decay_per_hour:.1%}\n"
                    f"⏰ {outcome.revival_hours:.0f} 小时后可自然复活（永久损失 {outcome.exp_loss_rate:.0%} 修为）\n"
                    f"━━━━━━━━━━━━━━━\n"
                    f"💡 复活方式：\n"
                    f"1. 等待自然复活（发送「自然复活」，损失 {outcome.exp_loss_rate:.0%} 修为）\n"
                    f"2. 使用还魂丹（无损失，详见后续炼丹玩法）\n"
                    f"3. 请求宗门（宗主/长老）或师父救援\n"
                    f"━━━━━━━━━━━━━━━\n"
                    f"发送「元神状态」可查看元神详情"
                )
                ...
                logger.info(
                    ...
                    f"{current_level_name} -> {next_level_name}，死亡概率 {death_rate:.2%}，"
                    f"自然复活需 {outcome.revival_hours:.0f} 小时"
                )

...
MATERIAL_REFINING_VALUES = {
    ...
    "仙器碎片": (30000, 120000),
    # === v3.11.0：世界事件材料纳入炼化 ===
    # 此前世界事件掉的妖兽内丹 / 魔核 / 魔晶等既不是任何丹方原料，也不在炼化白名单里，
    # 玩家拿到后毫无用途（「顶着死亡风险只掉俩魔核」的核心原因之一）。
    # 取值按 items.json 的价格换算：灵石 ≈ 价格 × 0.4，修为 = 灵石 × 4，与上表口径一致。
    "妖兽皮毛": (150, 600),
    "邪修令牌": (500, 2000),
    "血灵珠": (600, 2400),
    "妖兽内丹": (2000, 8000),
...
ITEM_CATEGORIES = {
    "材料": ["灵草", "精铁", "玄铁", "星辰石", "灵石碎片", "灵兽毛皮", "灵兽内丹",
             ...
             "远古秘籍", "仙器碎片",
             # 世界事件材料（v3.11.0：保证物品索引缺失时也能归到「材料」分类）
             "妖兽皮毛", "邪修令牌", "血灵珠", "妖兽内丹", "嗜血草", "魔核",
             "太古龙骨", "灵髓精华", "月华精粹", "天魔精华",
             # 秘境材料（Lv.1-8）

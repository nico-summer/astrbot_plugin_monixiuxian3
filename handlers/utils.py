...
SOUL_STATE_ALLOWED_COMMANDS = [
    ...
    "世界事件",
    "世界事件战绩",
    # v3.11.0 修复：这两条是 v3.10.0 新增的只读查询，却漏登记进元神白名单，
    # 导致「在世界事件里阵亡 -> 想看自己怎么死的 -> 被元神状态拦下」这种荒谬体验
    # （指令名见 managers/world_event_manager.py 的 CMD_WORLD_EVENT_BATTLE /
    # CMD_WORLD_EVENT_REWARDS；此处按本文件既有风格用字面量，避免 utils 反向依赖 managers）
    "世界事件战报",
    "我的世界事件奖励",
    # 传承系统的只读查询（同样是被漏掉的只读指令）
    "传承信息",
    "传承排行",
    "突破信息",
    "我的队伍",

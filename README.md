# AstrBot 插件：模拟修仙 v2

<p align=”center”>
  <img src=”logo.png” alt=”模拟修仙” width=”200”>
</p>

> **版本:** v3.2.3
> **许可证:** AGPL-3.0  
> **作者:** n1ko
> **基于:** [nonebot_plugin_xiuxian_2](https://github.com/xiuxian-2/nonebot_plugin_xiuxian_2) (部分借鉴与重构)

一款为 AstrBot 设计的、功能丰富的放置类修仙游戏插件。让你在聊天群中体验从凡人到大能的修行之路，工作摸鱼、畅聊吹水的同时亦可得道飞升！

---

## 🌟 特色功能

### 核心玩法
| 系统 | 说明 |
|------|------|
| **境界修炼** | 练气→筑基→金丹→元婴→化神→炼虚→合体→大乘→渡劫，九大境界逐步突破 |
| **战斗系统** | HP/MP/ATK/暴击/防御 真实计算，支持 PVP（切磋/决斗）和 PVE |
| **宗门系统** | 创建/加入宗门，职位管理，宗门建设与捐献 |
| **世界Boss** | 全服Boss降临，多人合力讨伐，按伤害排名瓜分奖励 |
| **🆕 秘境探索** | 刷装备/功法/稀有材料副本，爆率玩法（25%-40%装备，10%-20%功法） |
| **历练系统** | 日常材料农场，获取炼丹材料+经验灵石，稳定产出 |
| **炼丹系统** | 收集材料炼制丹药，提升修为或增加属性 |
| **传承系统** | 传承挑战争夺Buff加成，传承排行榜竞技 |

### 独有系统
| 系统 | 说明 |
|------|------|
| **🏔️ 洞天福地** | 个人修炼空间，5档洞天可选，持续产出灵石和修为，支持转让换购 |
| **🌾 灵田种植** | 开垦灵田种植灵草，5种灵草不同收益周期 |
| **💕 双修系统** | 与他人双修互增10%修为，1小时冷却 |
| **👁️ 天地灵眼** | 抢占灵眼持续获益，PVP资源争夺玩法 |
| **👨‍🏫 师徒系统** | 高境界玩家收徒传道，获得徒弟修炼收益；低境界玩家拜师，获得灌顶助力 |
| **🤝 组队秘境** | 🆕 2-4人组队探索高难度秘境，队伍平均等级准入，探索时间缩短，掉落加成，3个组队专属副本 |

---

## 📜 指令大全

### 📖 入门 & 基础
| 指令 | 说明 |
|------|------|
| `我要修仙 [灵修/体修]` | 创建角色，灵修擅法术，体修擅肉身 |
| `我的信息` | 显示境界、修为、战力、装备与已装备功法效果 |
| `签到` | 每日领取灵石(50-200)和修为奖励 |
| `改道号 <名称>` | 修改你的仙道称号 |
| `弃道重修 [确认]` | 删除角色并清空数据，7天内仅可执行一次，需追加”确认” |

### 🧘 修炼突破
| 指令 | 说明 |
|------|------|
| `闭关` | 开始闭关修炼，离线也可积累修为 |
| `出关` | 结算闭关时间，获取对应修为 |
| `突破信息` | 查看当前境界突破所需修为、成功率、风险 |
| `突破 [丹药名]` | 尝试突破，可服用丹药增加成功率 |

### 🎒 装备丹药
| 指令 | 说明 |
|------|------|
| `我的装备` | 显示已装备的武器/防具/心法/功法及属性 |
| `装备 <名>` | 从储物戒装备到身上；普通功法最多同时装备3个 |
| `卸下 <名>` | 卸下装备放回储物戒 |
| `丹药背包` | 查看拥有的丹药及效果 |
| `服用丹药 <名>` | 使用丹药获得增益 |

> 功法通过百宝阁购买后存入储物戒；使用 `装备 <功法名>` 装备，最多同时装备3个。功法提供法伤、物伤、防御、精神力、灵气/气血容量或寿命等被动属性，装备后会计入 `我的信息`、战斗和战力计算。

### 🏪 商店系统
| 指令 | 说明 |
|------|------|
| `丹阁` | 丹药专卖(破境丹/修为丹/功能丹) |
| `器阁` | 装备专卖(武器/防具/饰品) |
| `百宝阁` | 综合商店(功法/材料/杂物) |
| `购买 <物品名/短码> [数量]` | 从当前商店购买物品，支持市场列表短码 |
| `物品信息 <名称>` | 查看任意上架物品/丹药的具体效果与获取途径 |
| `储物戒` | 查看你的储物空间和物品（分类显示） |
| `存入 <物品名> [数量]` | 存入物品到储物戒 |
| `取出 <物品名> [数量]` | 从储物戒取出物品 |
| `搜索物品 <关键词>` | 模糊搜索储物戒物品 |
| `取出所有 <分类>` | 批量取出指定分类（材料/装备/功法/其他） |
| `炼化材料 <材料名> [数量/全部]` | 将 Boss 材料消耗并转换为灵石与修为 |
| `炼化图鉴` | 查看 Boss 材料的炼化价值 |
| `更换储物戒 <名称>` | 升级储物戒（需支付灵石） |
| `赠予 <@某人> <物品> [数量]` | 将物品赠送给其他玩家 |

### 🏦 灵石银行
年利率5%，每日复利

| 指令 | 说明 |
|------|------|
| `银行` | 查看存款余额和待领利息 |
| `存灵石 <数量>` | 存入灵石，开始计息 |
| `取灵石 <数量>` | 取出存款 |
| `领取利息` | 每日可领一次利息 |

### 📜 悬赏任务
| 指令 | 说明 |
|------|------|
| `悬赏令` | 查看当期多难度悬赏（显示风险/目标/奖励/说明） |
| `接取悬赏 <编号>` | 接取指定任务，限时内完成对应目标 |
| `悬赏状态` | 查看任务进度、剩余时间和奖励 |
| `完成悬赏` | 提交任务领取灵石/修为/掉落 |
| `放弃悬赏` | 取消当前任务（触发冷却） |
> 悬赏进度通过对应标签的活动累计：巡山/云游/猎魔等历练路线会提供不同“冒险标签”，秘境探索也能推进探索类悬赏。

### 🏛️ 宗门系统
| 指令 | 说明 |
|------|------|
| `创建宗门 <名>` | 消耗灵石创建宗门 |
| `加入宗门 <名>` | 加入已有宗门 |
| `我的宗门` | 查看宗门信息和成员（含宗门修炼加成） |
| `宗门列表` | 查看所有宗门排名 |
| `退出宗门` | 退出当前宗门 |
| `宗门捐献 <数量>` | 捐献灵石增加宗门建设度 |
| `宗门功法库` | 🆕 查看宗门功法库列表 |
| `捐献功法 <功法名>` | 🆕 将功法捐献到宗门（宗主/长老专属） |
| `借用功法 <功法名>` | 🆕 借用宗门功法（7天期限，最多2个） |
| `宗门任务` | 🆕 查看宗门每日任务完成情况 |
| `宗门秘境 <1-3>` | 🆕 进入宗门专属秘境（每日1次，消耗贡献度） |

**🆕 宗门福利系统：**
- **修炼加成**：根据宗门建设度提供修炼速度加成（5%-30%）
  - 1-5000：初级宗门 +5%
  - 5001-20000：中级宗门 +10%
  - 20001-50000：高级宗门 +15%
  - 50001-100000：顶级宗门 +20%
  - 100001+：超级宗门 +30%
- **功法共享**：高职位成员捐献功法，低职位成员可借用（7天自动归还）
- **宗门秘境**：专属秘境，奖励更丰厚（奖励提升50%）
  - 建设度10000+：解锁"宗门试炼地"（金丹期推荐）
  - 建设度30000+：解锁"宗门秘藏"（元婴期推荐）
  - 建设度60000+：解锁"宗门禁地"（化神期推荐）
- **每日任务**：完成任务获得额外贡献度
  - 捐献灵石（1000灵石，+50贡献度）
  - 完成历练（任意历练，+30贡献度）
  - 种植灵草（收获灵田，+20贡献度）
  - 探索秘境（完成秘境，+40贡献度）

### ⚔️ 战斗竞技
| 指令 | 说明 |
|------|------|
| `切磋 <@某人>` | 友好切磋，无损失 |
| `决斗 <@某人>` | 败者负伤1小时；胜者获得随机灵石或胜势Buff |
| `世界Boss` | 查看Boss信息和血量 |
| `挑战Boss` | 失败奖励即时到账；击杀后按全员累计伤害瓜分灵石并获得物品掉落 |
| `传承挑战 <@某人>` | 争夺对方传承加成 |
| `传承排行` | 查看传承榜 |

### 📊 排行榜
| 指令 | 说明 |
|------|------|
| `境界排行` | 查看境界排行榜 |
| `战力排行` | 查看战力排行榜 |
| `灵石排行` | 查看灵石排行榜 |
| `宗门排行` | 查看宗门排行榜 |

### 🚶 历练系统
| 指令 | 说明 |
|------|------|
| `开始历练 <路线名>` | 巡山问道 / 云游四方 / 猎魔肃清 / 九死一生 |
| `历练状态` | 查看当前路线进度与剩余时间 |
| `完成历练` | 领取路线奖励与奇遇掉落 |
| `历练信息` | 查看路线风险、可玩性与收益说明 |

### 🌀 秘境探索
| 指令 | 说明 |
|------|------|
| `秘境列表` | 查看15个主题秘境及奖励 |
| `探索秘境 <ID>` | 进入秘境（30-60分钟） |
| `完成探索` | 领取奖励：⚔️装备(25%-65%) 📜功法(10%-45%) 📦材料(保底) 🔥双倍(5%) |
| `退出秘境` | 放弃探索(无奖励) |

**秘境定位：** 刷装备/功法/稀有材料副本，爆率玩法
- **Lv.1 炼气期**（2个）：剑冢遗迹、灵兽巢穴 - 间隔拉大，避免过密
- **Lv.2 筑基期**（2个）：丹师洞府、玄冰洞窟
- **Lv.3 金丹期**（2个）：天火秘境、雷霆试炼地
- **Lv.4 元婴期**（2个）：幽冥血海、星辰殿堂
- **Lv.5 化神-炼虚期**（2个）：九幽黄泉路、上古战场
- **Lv.6 合体-大乘期**（2个）：混沌神殿、万界战场
- **Lv.7 渡劫期**（1个）：九天劫雷场
- **Lv.8 仙境**（2个）：仙道试炼、诸天战场 🆕

**等级限制：**
- 最低等级：必须达到秘境要求境界
- 等级过高警告：超过推荐等级10级，掉落率降低
- 等级过高禁止：超过推荐等级20级，无法进入

### 🔥 炼丹系统
| 指令 | 说明 |
|------|------|
| `丹药配方` | 查看炼丹配方及成功率，显示材料获取途径 |
| `炼丹 <配方ID>` | 炼制丹药（成功后存入丹药背包） |
| `材料查询 <材料名>` | 查看材料详细获取途径（历练/灵田） |

### ✨ 传承系统
| 指令 | 说明 |
|------|------|
| `传承信息` | 查看传承给你的属性加成 |

### 🏔️ 洞天福地
| 指令 | 说明 |
|------|------|
| `我的洞天` | 查看洞天信息和待收取收益 |
| `购买洞天 <1-5>` | 1:小洞天 2:中洞天 3:大洞天 4:福地 5:洞天福地 |
| `升级洞天` | 提升洞天等级增加收益 |
| `洞天收取` | 收取灵石和修为产出 |
| `转让洞天` | 转让当前洞天（回收价=基础50%+升级70%），可购买更高级洞天 |

**洞天类型：**
| 类型 | 价格 | 修炼加成 |
|------|------|----------|
| 小洞天 | 10,000灵石 | +5% |
| 中洞天 | 50,000灵石 | +10% |
| 大洞天 | 200,000灵石 | +20% |
| 福地 | 500,000灵石 | +30% |
| 洞天福地 | 1,000,000灵石 | +50% |

### 🌾 灵田种植
| 指令 | 说明 |
|------|------|
| `我的灵田` | 查看灵田和种植情况 |
| `开垦灵田` | 花费灵石开垦灵田 |
| `种植 <灵草名>` | 种植灵草 |
| `收获` | 收获成熟的灵草获取修为 |
| `升级灵田` | 增加种植格数 |

**可种植灵草：**
| 灵草 | 成熟时间 | 修为收益 |
|------|----------|----------|
| 灵草 | 1小时 | +500 |
| 血灵草 | 2小时 | +1,500 |
| 冰心草 | 4小时 | +4,000 |
| 火焰花 | 8小时 | +10,000 |
| 九叶灵芝 | 24小时 | +30,000 |

### 💕 双修系统
| 指令 | 说明 |
|------|------|
| `双修 <@某人>` | 向对方发起双修请求 |
| `接受双修` | 接受双修，双方各获10%修为 |
| `拒绝双修` | 拒绝对方请求 |

### 👨‍🏫 师徒系统
| 指令 | 说明 |
|------|------|
| `收徒 <@某人>` | 境界高10级以上可收徒，最多3个徒弟 |
| `拜师 <@某人>` | 向高境界玩家拜师 |
| `师徒信息` | 查看师徒关系和收益 |
| `灌顶 <@徒弟>` | 师父传授修为给徒弟（1小时冷却） |
| `逐出师门 <@徒弟>` | 师父逐出徒弟 |
| `离开师门` | 徒弟自主离开 |

### 🤝 组队秘境系统
| 指令 | 说明 |
|------|------|
| `创建队伍` | 创建一个新队伍 |
| `邀请入队 <@某人>` | 邀请玩家加入队伍 |
| `接受组队` | 接受队伍邀请 |
| `拒绝组队` | 拒绝队伍邀请 |
| `队伍信息` | 查看当前队伍成员和状态 |
| `离开队伍` | 离开当前队伍 |
| `踢出队伍 <@某人>` | 队长踢出队员 |
| `解散队伍` | 队长解散队伍 |
| `组队探索 <秘境ID>` | 队长发起组队探索（需2-4人） |
| `完成组队` | 队长完成组队探索并结算 |
| `队伍掉落` | 查看队伍待分配物品 |
| `分配物品 <编号> <@某人>` | 队长分配掉落物品 |

**组队机制：**
- **准入规则**：队伍平均等级 ≥ 秘境要求等级
- **探索时间**：2人90%，3人80%，4人70%
- **掉落加成**：2人+20%，3人+35%，4人+50%
- **经验分配**：所有队员获得100%经验（不分摊）
- **传承加成**：低等级队员获得额外经验（队友高10级+30%，高20级+50%）
- **次数限制**：每人每日每个秘境5次（单人和组队共享）

**组队专属秘境：**
| 秘境 | 等级要求 | 推荐人数 | 特色 |
|------|----------|----------|------|
| 修罗战场 | 元婴期(16) | 2-4人 | 极品装备、传说功法 |
| 九幽深渊 | 化神期(22) | 3-4人 | 传说装备、神器 |
| 天道试炼 | 炼虚期(28) | 4人满编 | 神器装备、至尊功法 |
| `收徒 <@某人>` | 元婴期以上可收徒，最多3名 |
| `拜师 <@某人>` | 金丹期以下可拜师 |
| `师徒信息` | 查看师徒关系和灌顶冷却 |
| `灌顶 <@徒弟>` | 消耗30%修为传授徒弟（24小时冷却） |
| `逐出师门 <@徒弟>` | 解除师徒关系 |
| `离开师门` | 主动离开师门 |

**师徒系统特色：**
- 🎓 **收徒条件**：元婴期以上，最多收3个徒弟
- 👶 **拜师条件**：金丹期以下，每人只能有1个师父
- 💰 **传道奖励**：徒弟修炼、历练时，师父自动获得8%收益
- ✨ **灌顶功能**：师父每日可传授修为助徒弟快速成长
- 🎁 **出师奖励**：徒弟达到筑基期自动出师，师父获得丰厚奖励（5万灵石+稀有材料）

### 🗡️ 弃道重修
| 指令 | 说明 |
|------|------|
| `弃道重修` | 展示重修提示，需处于空闲且无贷款状态 |
| `弃道重修 确认` | 立即删除角色数据并进入7天冷却期 |

> 重修可用于测试突破死亡逻辑或重新投胎，但冷却未结束前不可再次使用，且需要重新发送「我要修仙」创建角色。

### 👁️ 天地灵眼
| 指令 | 说明 |
|------|------|
| `灵眼信息` | 查看灵眼及可抢占列表 |
| `抢占灵眼 <ID>` | 占据无主灵眼 |
| `灵眼收取` | 收取灵眼修为产出 |
| `释放灵眼` | 释放已占据的灵眼 |

---

## 🎁 道具与丹药手册

| 类别 | 示例 | 作用 | 获取方式 |
|------|------|------|------|
| 破境丹 | 筑基丹、凝婴丹、化神丹 | 直接提升指定境界的突破成功率，部分丹药可将总成功率拉满 | 丹阁刷新、秘境稀有掉落 |
| 修为丹 | 炼气丹、玄灵丹、九转灵丹 | 立即增加修为值（200-10万+），全部存入丹药背包可随时服用 | 丹阁、炼丹系统、历练/秘境结算奖励 |
| 功能丹/灵丹 | 修炼加速丹、力量丹、灵智丹、回生丹 | 包含临时修炼倍率、永久属性增减、复活、定魂护盾等特殊效果 | 丹阁稀有刷新、Boss与高阶秘境掉落 |
| 百宝丹药（即时消耗） | 一品气血丹、三品凝神增益丹 | 购买后立即恢复气血/灵气、增减属性或短暂提升突破成功率 | 百宝阁限量出售（无法存入背包） |
| 材料 | 灵草、玄铁、星辰石、天材地宝 | 炼丹配方、悬赏任务、装备升级所需的基础资源 | 历练、秘境、悬赏、世界Boss掉落，灵田收获可持续产出灵草 |
| 法器/功法 | 青锋剑、玄铁甲、焚天诀、他化自在大法 | 装备/心法提供物伤、法伤、防御与修炼倍率等长期加成 | 器阁 / 百宝阁刷新，高级 Boss 亦可掉落稀有法器 |

💡 使用 `物品信息 <名称>` 可随时查询任意上架物品或丹药的详细效果与推荐获取途径。

---

## ⚙️ 配置说明

插件首次运行后，将在 `data/plugins/astrbot_plugin_monixiuxian2_niko/config/` 目录下自动生成默认配置文件：

| 配置文件 | 作用 |
|---------|------|
| `sect_config.json` | 宗门创建消耗、职位权限、任务奖励 |
| `boss_config.json` | Boss 属性成长曲线、生成间隔 |
| `rift_config.json` | 秘境名称、等级要求、探索时长、奖励池 |
| `alchemy_config.json` | 丹药配方、材料消耗、成功率计算 |

**注意**：修改配置后需重启 AstrBot 生效。

---

## 🖼️ 开启图片卡片 (可选)

本插件支持生成精美个人信息卡片：

1. **安装依赖**: `pip install Pillow`
2. **获取素材**: 下载[卡图资源包](https://cowtransfer.com/s/82b90d2b879d43) (口令：k3jzr5)
3. **放置素材**: 解压后将 `xiuxian` 文件夹放入 AstrBot 的 `data/` 目录中
4. **生效验证**: 发送 `/我的信息`，若配置正确将发送图片

---

## 🛠️ 安装与部署

1. 确保已安装 AstrBot
2. 将插件文件夹放入 `data/plugins/` 目录
3. 安装依赖：`pip install -r requirements.txt`
4. 在 AstrBot 控制台启用插件
5. 发送 `我要修仙` 开始体验！

---

## 📁 项目结构

```
astrbot_plugin_monixiuxian2/
├── main.py              # 插件入口
├── models.py            # 数据模型
├── config_manager.py    # 配置管理
├── handlers/            # 指令处理器
│   ├── player_handler.py        # 玩家基础
│   ├── breakthrough_handler.py  # 突破系统
│   ├── combat_handlers.py       # 战斗系统
│   ├── sect_handlers.py         # 宗门系统
│   ├── boss_handlers.py         # Boss系统
│   ├── rift_handlers.py         # 秘境系统
│   ├── adventure_handlers.py    # 历练系统
│   ├── alchemy_handlers.py      # 炼丹系统
│   ├── bank_handlers.py         # 银行系统
│   ├── bounty_handlers.py       # 悬赏系统
│   ├── shop_handler.py          # 商店系统
│   ├── blessed_land_handlers.py # 洞天福地
│   ├── spirit_farm_handlers.py  # 灵田系统
│   ├── spirit_eye_handlers.py   # 灵眼系统
│   └── dual_cultivation_handlers.py # 双修系统
├── managers/            # 业务逻辑管理器
├── config/              # 配置文件
├── data/                # 数据文件
└── utils/               # 工具函数
```

---

## 🤝 致谢

- **原版参考**: [NoneBot-Plugin-Xiuxian-2](https://github.com/xiuxian-2/nonebot_plugin_xiuxian_2)
- **原 AstrBot 插件维护者**: [linjianyan0229](https://github.com/linjianyan0229)

---

## 📝 更新日志

### v3.2.3 - 秘境每日次数限制
- **新增秘境每日次数限制**：每个秘境每天最多探索5次，防止数值膨胀
- **秘境列表显示剩余次数**：实时显示每个秘境今日剩余探索次数
- **探索完成提示剩余次数**：完成探索后显示该秘境今日剩余次数
- **每日00:00自动重置**：次数计数自动重置，无需手动操作
- **数据库迁移v25**：新增 `rift_daily_count` 和 `rift_count_reset_date` 字段

### v3.2.2 - 秘境分布优化与仙境秘境
- **秘境分布优化**：低级秘境间距拉大（ID 2: level 3→9），高级秘境重新分布
- **新增2个仙境秘境**：ID 14（仙道试炼，地仙 level 31）、ID 15（诸天战场，天仙 level 32）
- **掉落表扩展**：新增Lv.8材料（大罗金精、鸿蒙紫气）、装备（仙品/大罗）、功法（仙界传承/圣级传承）
- **秘境总数**：从13个扩展到15个，覆盖炼气期→天仙全境界
- **掉落率提升**：装备 25%-60% → 25%-65%，功法 10%-40% → 10%-45%

### v3.2.1 - 秘境等级限制机制
- **新增秘境等级限制**：防止高等级玩家刷低级秘境
  - 等级过低：无法进入
  - 等级适中（±10级）：正常掉落
  - 等级偏高（11-20级）：掉落率降低（最低至10%），警告提示
  - 等级过高（>20级）：禁止进入
- **掉落惩罚公式**：`drop_penalty = max(0.1, (20 - level_diff) / 10)`
- **用户体验优化**：进入时显示警告，列表显示境界要求

### v3.2.0 - 秘境系统全面重构
- **秘境数量扩展**：从5个扩展到13个，覆盖炼气期→渡劫期
- **三层掉落系统**：
  - 🔥 材料掉落：100%保底1-2个稀有材料
  - ⚔️ 装备掉落：25%-60%爆率，随秘境等级提升
  - 📜 功法掉落：10%-40%爆率，随秘境等级提升
  - 🎲 双倍掉落：5%概率触发所有掉落翻倍
- **掉落物自动分类存储**：装备/功法→储物戒，材料→储物戒，丹药→丹药背包
- **8级秘境体系**：Lv.1-8对应炼气期到渡劫期，每级独立掉落表
- **物品质量进阶**：从普通→极品→传说→神器→至尊，覆盖全境界
- **增量初始化**：自动检测并添加缺失秘境，无需删除旧数据

### v3.1.16 - 完善装备属性与指令兼容
- 修复装备失败后的异常返还分支，避免玩家数据不存在时再次触发空对象错误。
- `卸下` 指令兼容自定义唤醒词及命令参数残留前缀。
- 战斗属性统一读取装备管理器，旧格式法器的攻击/防御属性也会计入战斗。

### v3.1.15 - 修复功法显示与装备流程
- `我的信息` 增加普通功法列表及每个功法的被动属性展示。
- 修复 `装备` 指令在自定义唤醒词下参数解析异常，以及取出装备后旧玩家对象覆盖储物戒数据的问题。
- 统一旧格式法器的装备类型和攻击/防御属性解析，修复旧格式武器、防具无法正确装备或属性不生效的问题。
- 修复商店对旧格式法器类型的识别，购买后可正常存入储物戒并装备。

### v3.1.14 - 修复赠予指令解析
- 修复自定义 `=` 唤醒前缀与消息链中的赠予命令被当作物品名的问题。
- 赠予指令优先使用命令参数，并兼容 At、QQ号、多空格和常见平台文本格式。
- 赠予请求创建失败时自动返还已扣除的物品，避免物品丢失。

### v3.1.13 - 修复丹药与功法效果展示
- 修复永久修炼倍率丹药突破境界后失效的问题。
- 为功法补充实际属性效果并接入决斗属性计算，完善获取、装备说明。
- 统一丹药信息为实际结算字段，并补齐狂乱丹、洗髓丹、易筋丹等特殊效果。
- “我的信息”补充显示永久丹药增益与临时丹药效果。

### v3.1.12 - 完善 Boss 掉落物用途

- 新增 `炼化材料` 与 `炼化图鉴` 指令，Boss 掉落的十种材料均可明确炼化为灵石和修为。
- 炼化按单件固定价值结算，支持指定数量或全部炼化；扣除材料与发放收益在同一数据库事务中完成。

### v3.1.11 - 修复世界Boss伤害与奖励结算

- 修复每次挑战重复累计Boss历史损血、导致多人伤害占比总和超过100%的问题；伤害榜现在按全体参与者实际累计伤害计算占比。
- 修复挑战失败时安慰奖仅显示但未到账，以及最终击杀者的灵石分成和物品掉落可能被旧玩家数据覆盖的问题。
- Boss击杀结算覆盖全部参与者并完整分配奖励池，不再只向伤害榜前20名分配奖励；同时串行处理挑战，避免并发重复扣血或重复结算。

### v3.1.10 - 修复插件唯一标识冲突

- 将插件唯一标识调整为 `astrbot_plugin_monixiuxian2_niko`，避免与 AstrBot 插件市场中的原版同名插件冲突。
- 同步调整插件数据目录标识；迁移旧数据时请将数据库复制到新目录 `data/plugin_data/astrbot_plugin_monixiuxian2_niko/`。

### v3.1.9 - 完善决斗奖励

- 决斗胜者可从败者处获得随机 100–1000 灵石。
- 败者灵石不足时，胜者获得 1 小时攻击与防御提升 20% 的胜势 Buff。

### v3.1.7 - 修复插件循环导入

### v3.1.8 - 修复战斗HP与世界Boss奖励

- 修复战败后HP锁定为1；新增`恢复`指令，HP耗尽后必须恢复才能再次挑战Boss。
- `我的信息`显示战斗HP；世界Boss记录伤害榜并在击败后按伤害比例分配灵石。

修复插件初始化时 `ShopManager` 因 `ConfigManager` 与数据库迁移模块互相导入而无法加载的问题。

### v3.1.6 - 修复商店短码购买

修复“市场/丹阁/器阁/百宝阁”列表未显示短码、导致按短码购买失败的问题。现在每件商品都会显示唯一短码，`购买` 命令支持使用商品名称或短码，并兼容旧商店数据自动补齐短码。

### v3.1.5 - 修复扩展功能数据表缺失

修复初始化数据库缺少洞天福地、灵田、双修和天地灵眼数据表的问题。启动插件时会自动创建缺失表并初始化天地灵眼数据。

### v3.1.4 - 修复闭关数据库错误

修复旧版或全新数据库缺少 `user_cd.extra_data` 字段，导致闭关指令报错的问题。启动插件时会自动补齐字段，无需删除或手动修改数据库。

### v2.9.3 - 排行榜系统全面优化

**🔴 BUG修复**
| 问题 | 修复位置 | 说明 |
|------|----------|------|
| 战力排行榜与玩家信息战力不一致 | ranking_manager.py | 统一战力计算公式为：物伤+法伤+物防+法防+精神力/10 |
| 战力排行未考虑装备加成 | ranking_manager.py | 现在正确计算装备和丹药效果后的总属性 |
| 境界排行显示Lv.X而非境界名称 | ranking_manager.py | 现在显示实际境界名称（如"玄体境初期"） |
| user_id非字符串时切片报错 | ranking_manager.py | 封装`_short_id()`安全方法 |
| 宗门职位索引越界崩溃 | ranking_manager.py | 使用`POSITION_MAP`安全映射 |
| 宗门排行依赖DB隐式排序 | ranking_manager.py | 显式按建设度排序 |

**✨ 新功能**
| 功能 | 说明 |
|------|------|
| 贡献排行 | 新增`贡献排行`指令，查看当前宗门成员贡献度排行 |

**🛡️ 安全性优化**
| 优化项 | 说明 |
|------|------|
| 名称长度截断 | 道号/宗门名最长12字符，防止刷屏 |
| 特殊字符过滤 | 过滤`@`符号，防止触发群通知 |
| 统一名称处理 | 封装`_safe_name()`方法，所有排行榜统一使用 |

**⚡ 性能优化**
| 优化项 | 说明 |
|------|------|
| EquipmentManager复用 | 初始化时创建一次，避免每次查询重复创建 |

---

### v2.9.2 - 排行榜战力计算修复

**🔴 BUG修复**
| 问题 | 修复位置 | 说明 |
|------|----------|------|
| 战力排行榜与玩家信息战力不一致 | ranking_manager.py | 统一战力计算公式为：物伤+法伤+物防+法防+精神力/10 |
| 战力排行未考虑装备加成 | ranking_manager.py | 现在正确计算装备和丹药效果后的总属性 |
| 战力排行显示ATK而非实际攻击属性 | ranking_manager.py | 根据修炼类型显示物伤(体修)或法伤(灵修) |

---

### v2.9.1 - 系统行为互斥性修复

**🔴 高严重性修复 (状态互斥)**
| 问题 | 修复位置 | 说明 |
|------|----------|------|
| 双重状态系统不同步 | player_handler.py | 闭关/出关时同步更新user_cd表 |
| 历练期间可执行突破 | utils.py | @player_required装饰器增加user_cd状态检查 |
| 历练/秘境期间可发起战斗 | combat_handlers.py | 添加发起者和目标的状态检查 |
| 秘境期间可炼丹 | alchemy_handlers.py | 添加玩家存在和状态检查 |
| 历练期间可加入/退出宗门 | sect_handlers.py | 关键操作添加状态检查 |

**✨ 改进**
- 统一状态检查机制：`@player_required`装饰器现在同时检查`player.state`和`user_cd.type`
- 新增45+条命令白名单，允许在忙碌状态下执行查看类操作（信息查看、银行、背包、排行榜等）
- 状态提示优化：显示具体状态名称（历练中/探索秘境中/宗门任务中等）

---

### v2.9.0 - 安全性与稳定性大修复

**🔴 高严重性修复 (P0)**
| 问题 | 修复位置 | 说明 |
|------|----------|------|
| 储物戒物品来源验证漏洞 | storage_ring_handler.py | 禁用手动存入，物品只能通过系统自动存入 |
| HP/MP计算不一致 | combat_handlers.py | 统一使用 CombatManager.calculate_hp_mp 返回值 |
| 贷款逾期检查并发风险 | utils.py | 添加事务保护防止重复删除玩家 |
| 级联删除不完整 | data_manager.py | 添加 pending_gifts 表的级联删除 |
| 储物戒操作无事务保护 | storage_ring_manager.py | store/retrieve/discard 添加事务保护 |

**🟡 中严重性修复 (P1)**
| 问题 | 修复位置 | 说明 |
|------|----------|------|
| 防御减伤公式不合理 | combat_manager.py | 改用递减公式 defense/(defense+100) |
| 银行利息精度问题 | bank_manager.py | 使用 Decimal 精确复利计算 |
| JSON解析异常处理宽泛 | models.py | 改为 except json.JSONDecodeError |
| 定时任务异常恢复不健壮 | main.py | 添加指数退避重试机制 |
| 购买流程事务不完整 | shop_handler.py | 整个购买流程包装在单一事务中 |

**🟢 低严重性修复 (P2)**
| 问题 | 修复位置 | 说明 |
|------|----------|------|
| 闭关上限固定24小时 | player_handler.py | 根据境界动态调整（24h-72h） |
| 银行配置硬编码 | bank_manager.py | 配置外部化，支持自定义利率等参数 |

**✨ 改进**
- 防御公式优化：100防御=50%减伤，200防御=66%减伤，更合理的递减曲线
- 闭关时长：练气24h → 筑基30h → 金丹36h → ... → 渡劫72h
- 银行配置可在 config 中自定义：利率、存款上限、贷款额度等

---

### v2.7.1 - Bug修复与系统完善

**🔧 Bug修复**
| 问题 | 修复 |
|------|------|
| 炼丹成功后丹药直接使用 | 现在正确存入丹药背包，需手动服用 |
| 赠予指令At解析失败 | 修复At组件解析，兼容多种平台格式 |
| 炼丹配方与配置不匹配 | 配方丹药改为配置文件中实际存在的丹药 |
| 秘境丹药掉落丢失 | 丹药掉落现在正确存入丹药背包而非储物戒 |

**✨ 改进**
- 炼丹成功提示优化：显示"丹药背包"而非"储物戒"
- 删除alchemy_manager中的无效占位方法，统一使用pill_manager
- 炼丹配方更新为：炼气丹、聚灵丹、凝气丹、培元丹、玄灵丹

---

### v2.7.2 - BUG修复

**🔧 修复问题**
- 修复 `update_user_cd` 方法未保存 `extra_data` 字段的问题，导致秘境管理器等功能的额外数据丢失

---

### v2.7.0 - 灵石银行系统完善

🎯 核心改造：银行系统全面升级，新增贷款、流水、排行功能

🏦 银行新功能
| 功能 | 指令 | 说明 |
|------|------|------|
| 贷款 | `/贷款 <金额>` | 普通贷款：0.5%日利率，7天期限 |
| 突破贷款 | `/突破贷款 <金额>` | 高风险贷款：0.8%日利率，3天期限 |
| 还款 | `/还款` | 偿还当前贷款（本金+利息） |
| 银行流水 | `/银行流水` | 查看最近15条交易记录 |
| 存款排行 | `/存款排行` | 查看存款富豪榜 |

---

### v2.6.0 - 突破系统完善

**🎯 核心改造：突破丹药体系重构，全境界覆盖**

**💊 专属破境丹完善**
| 新增丹药 | 目标境界 | 加成效果 |
|---------|----------|---------|
| 固基丹·中/后 | 筑基期内部突破 | +50%/+60% |
| 温丹丹·中/后 | 金丹期内部突破 | +70%/+72% |
| 养婴丹·中/后 | 元婴期内部突破 | +76%/+74% |
| 温神丹·中/后 | 化神期内部突破 | +77%/+78% |
| 破虚丹·中/后 | 炼虚期内部突破 | +79%/+77% |
| 融合丹·中/后 | 合体期内部突破 | +72%/+70% |
| 悟道丹·中/后 | 大乘期内部突破 | +62%/+58% |
| 混元丹 | 混元大罗金仙 | +5% |

**🔄 通用增益丹体系**
- 原"三品筑基丹"等改名为"xx增益丹"，明确定位为通用突破辅助
- 无境界限制，购买后获得1小时临时突破成功率加成
- 可与专属破境丹叠加使用

**🌀 秘境稀有丹药掉落**
| 秘境等级 | 掉落概率 | 可掉落丹药 |
|----------|----------|-----------|
| 低级秘境 | 3% | 三品凝神增益丹 |
| 中级秘境 | 5% | 三/四/五品增益丹 |
| 高级秘境 | 10% | 四/五/六/七品增益丹 |

**🔧 修复问题**
- 修复items.json中`add_breakthrough_bonus`未生效的问题
- 通用增益丹购买后正确添加临时突破加成效果

---

### v2.8.2 - 游戏平衡性与玩法优化

**🔧 修复问题**
| 问题类型 | 修复位置 | 说明 |
|---------|---------|------|
| 战斗防御未应用 | combat_manager.py | PVP和Boss战中玩家防御力现已正确生效 |
| 洞天收益无上限 | blessed_land_manager.py | 添加每小时修为收益上限（5k-100k按洞天等级） |
| 历练收益无上限 | adventure_manager.py | 添加修为/灵石收益上限（按历练类型） |

**🆕 新增功能**
- 灵田枯萎机制：成熟作物48小时未收获将枯萎，增加游戏策略性

---

### v2.8.1 - 安全性与稳定性修复

**🔧 修复问题**
- 修复银行追杀时未级联删除玩家关联数据的问题（灵眼/洞天/灵田/银行等数据残留）

**✅ 已验证修复项**
| 问题类型 | 修复位置 | 说明 |
|---------|---------|------|
| 玩家死亡级联删除 | data_manager.py | 已实现 `delete_player_cascade()` |
| 银行并发事务 | bank_manager.py | 存取款使用 `BEGIN IMMEDIATE` 事务 |
| 双修修为差距限制 | dual_cultivation_manager.py | 限制双方修为差距最大3倍 |
| 灵眼抢占原子操作 | spirit_eye_manager.py | 事务+条件UPDATE防并发 |
| 宗主死亡处理 | sect_manager.py | 自动传位或解散宗门 |
| 装备属性完整应用 | combat_handlers.py | 防具defense已应用到战斗 |
| 悬赏过期处理 | bounty_manager.py | 完成时检查超时自动取消 |
| 宗门名称验证 | sect_manager.py | 长度(2-12字)和敏感词检查 |
| 传承buff应用 | combat_handlers.py | impart加成已应用到战斗 |

---

### v3.1.3 - 数据库连接自动恢复
- 修复世界Boss/灵眼等定时任务在数据库连接意外断开后持续报错 `no active connection` 的问题
- 新增数据库自动重连与定时任务保活逻辑，保证所有后台任务可自动恢复

### v3.1.2 - 悬赏时限自适应
- 根据历练路线的真实时长自动放宽悬赏时限，确保巡山/猎魔等任务在限定时间内可完成
- 历练路线映射的疲劳信息同时用于推算缓冲时间，避免路线无法连续接取导致的“必失败”任务

### v3.1.1 - 历练/悬赏问题修复
- 历练结算将根据路线限定的计划时长发放奖励，延迟领取不再无限放大收益
- 悬赏编号若超出缓存会提示玩家重新查看列表，避免接入错误任务导致进度无法推进
- 悬赏模板的进度标签统一转换为小写并写入任务记录，彻底修复大小写不一致造成的进度异常

### v3.1.0 - 历练路线与悬赏焕新
- 新增“巡山/云游/猎魔/九死一生”四条历练路线，区分风险、收益、休整与悬赏标签，奖励与奇遇完全可配置
- 历练完成提示路线与事件详情，奖励随境界、路线与事件联动，可额外掉落稀有素材
- 悬赏系统启用多难度模板，显示清晰目标/风险/奖励；不同活动标签（历练路线、秘境探索）推进对应任务
- 悬赏奖励引入难度倍率与自定义掉落表，完成后会展示难度并自动发放物品
- `/修仙帮助`、README、metadata 全量更新说明，便于新老玩家理解新版玩法

### v3.0.0 - 悬赏系统修复 & 新手友好

- 悬赏模板仅生成可通过历练/秘境推进的任务类型，避免出现无法完成的“护送/收集”任务
- 悬赏区新增说明，明确进度与历练/秘境挂钩
- 包含 v2.9.9 的弃道重修功能及道具信息改进

---

### v2.9.9 - 弃道重修 & 道具完善

- 新增「弃道重修」指令：7天冷却，可在空闲状态下手动删除角色测试数据
- 玩家可发送「弃道重修 确认」执行重置，防止误触
- README、帮助文本同步新指令说明

---

### v2.9.8 - 道具指南与查询指令

- 新增 `物品信息 <名称>` 指令，可查询任意在售物品/丹药的详细效果与推荐获取方式
- 商店展示的物品详情补充突破加成、修炼倍率、永久属性增减等关键信息
- README 新增《道具与丹药手册》板块，梳理所有可获得的道具类别与来源

---

### v2.9.7 - 突破死亡清理修复

**🛠 BUG 修复**
| 问题 | 修复 |
|------|------|
| 突破失败死亡时，级联删除命令若遇到旧版本缺少的系统表会报错，导致玩家数据无法清理 | `delete_player_cascade` 现采用容错执行，缺表/旧表不再阻断玩家删除，确保死亡后可重新创建角色 |

---

### v2.9.6 - 冷却与战斗解析修复

**🔧 修复问题**
| 问题 | 修复 |
|------|------|
| 双修冷却仅对发起者生效，接受者可无限刷收益 | 双修请求与接受流程现在都会检测双方冷却时间 |
| 决斗/切磋在部分平台无法解析 @ 目标 | 目标解析新增多属性兼容与纯文本回退，避免 AttributeError |

**📌 其他**
- README、帮助信息与 metadata 版本同步至 v2.9.6，记录上述修复

---

### v2.9.5 - 悬赏系统安全加固

**🔒 安全修复**
| 问题 | 严重程度 | 修复 |
|------|---------|------|
| 奖励发放与任务完成非原子操作 | 🔴严重 | 使用 `BEGIN IMMEDIATE` 事务保护，防止并发重复领奖 |
| 放弃任务无冷却机制 | 🟠中等 | 放弃悬赏后30分钟内无法接取新任务 |
| 并发领取风险 | 🟠中等 | 事务内先标记完成再发放奖励 |

**🛡️ 事务安全改进**
- `complete_bounty` 现在使用 SQLite 事务保护
- 奖励发放顺序：先标记任务完成 → 再发放灵石/修为 → 最后发放物品
- 物品奖励在事务外处理，失败不影响主奖励

---

### v2.9.4 - 商店购买物品存入储物戒 & 悬赏系统修复

**🔧 修复问题**
| 问题 | 修复 |
|------|------|
| 装备类物品购买后直接装备 | 现在购买的武器/防具/功法/饰品统一存入储物戒 |
| 体修信息面板缺少法伤显示 | 补充显示法伤属性 |
| 突破死亡时删除双修数据报错 | 修复SQL语句中的列名错误 |
| 悬赏任务可无限刷取 | 添加进度检查，必须通过历练/秘境完成任务目标 |

**🆕 悬赏系统改进**
- 悬赏任务现在需要通过实际游戏活动完成进度
- 历练完成：可推进击杀/采集/探索类悬赏
- 秘境探索：可推进探索类悬赏
- 完成历练/秘境后会显示悬赏进度提示

---

### v2.5.0 - 储物戒系统重构

**🎯 核心改造：储物戒成为游戏物品流转的中央枢纽**

**📦 系统联动（8个系统接入）**
| 系统 | 入库场景 | 出库场景 |
|------|---------|---------|
| 商店系统 | 购买物品→自动存入 | - |
| 装备系统 | 卸下装备→存入 | 装备→从储物戒取出 |
| 炼丹系统 | - | 炼丹→消耗储物戒材料 |
| 历练系统 | 完成历练→物品掉落 | - |
| 秘境系统 | 探索完成→物品掉落 | - |
| Boss系统 | 击杀奖励→物品掉落 | - |
| 悬赏系统 | 完成悬赏→物品奖励 | - |
| 灵田系统 | 收获灵草→存入储物戒 | - |

**🆕 新增功能**
- 物品分类视图：按材料/装备/功法/其他分组显示
- 物品搜索：`搜索物品 关键词` 模糊搜索
- 批量取出：`取出所有 分类` 批量操作
- 储物戒升级消耗：需支付灵石购买

**🔧 修复问题**
- 炼丹现在正确消耗储物戒中的灵草材料
- 装备物品必须先存在于储物戒中
- 赠予请求持久化到数据库（24小时有效期）

---

## 📋 更新日志

### v3.2.2 (2025-01-XX)

**🌀 秘境分布优化**
- 🎯 低级秘境间隔拉大：炼气期四层 → 十层，避免过于密集
- 🎯 高级秘境更均匀分布：合体期后期、大乘期后期各1个
- 🆕 新增2个仙境秘境：地仙（仙道试炼）、天仙（诸天战场）
- 📊 总计15个秘境，覆盖炼气期到天仙全境界

**💎 新增Lv.8仙境掉落**
- 材料：大罗金精、混元道石、天道碎片、鸿蒙紫气
- 装备：大罗剑·斩道、混元斧·开天、圣甲·不朽（65%爆率）
- 功法：大罗金仙诀、混元无极功、圣人传承（45%爆率）

**🔧 秘境名称优化**
- 虚空裂缝 → 九幽黄泉路（化神期）
- 仙陨战场 → 上古战场（炼虚期）
- 诸天万界塔 → 万界战场（大乘期）
- 仙界试炼地 → 九天劫雷场（渡劫期）

**📊 秘境等级分布**
```
炼气期(0-9)：2个秘境，间隔0→9
筑基期(10-12)：2个秘境，间隔10→12
金丹期(13-15)：2个秘境，间隔13→15
元婴期(16-18)：2个秘境，间隔16→18
化神-炼虚期(19-24)：2个秘境，间隔20→24
合体-大乘期(25-29)：2个秘境，间隔27→29
渡劫期(30)：1个秘境
仙境(31-32)：2个秘境，间隔31→32
```

### v3.2.1 (2025-01-XX)

**🌀 秘境系统完整重构**
- ✨ 重新定位：从"挂机副本"改为"装备/功法/稀有材料"刷宝副本
- 🎮 初始化13个主题秘境：覆盖炼气期到渡劫期全境界
  - Lv.1-3：原有5个秘境（剑冢、灵兽、丹师、玄冰、天火）+ 雷霆试炼地
  - Lv.4 元婴期：幽冥血海、星辰殿堂
  - Lv.5 化神-炼虚期：虚空裂缝、仙陨战场
  - Lv.6 合体-大乘期：混沌神殿、诸天万界塔
  - Lv.7 渡劫期+：仙界试炼地
- 💎 三大掉落系统：
  - 材料掉落（100%保底）：玄铁、星辰石、紫金神铁、仙晶等全阶材料
  - 装备掉落（25%-60%爆率）：从普通到仙品，随等级提升
  - 功法掉落（10%-40%爆率）：从基础到仙界传承
- 🎲 双倍掉落机制：5%概率触发，所有掉落翻倍
- 📊 分类显示：⚔️装备 📜功法 📦材料 🔥丹药 分类图标显示
- 🔧 解决"从未出现"问题：首次启动自动初始化秘境数据
- 🔒 等级限制系统：
  - 最低等级要求：必须达到秘境推荐境界
  - 等级过高警告：超过10级掉落率线性降低（11级差=90%，20级差=10%）
  - 等级过高禁止：超过20级无法进入，引导选择合适秘境

**🔥 炼丹系统优化**
- 📍 材料来源提示：配方列表显示材料获取途径（历练/灵田）
- 🔍 新增 `/材料查询` 指令：查看材料详细获取方法
- 💡 智能提示：材料不足时自动提示获取途径
- 🛠️ 集中管理：材料来源配置统一管理，避免重复代码

**🏔️ 洞天福地优化**
- 💰 新增 `/转让洞天` 功能：解决无法更换高级洞天的问题
- 📈 回收价格公式：基础购买价×50% + 升级总投入×70%
- 💎 透明展示：洞天信息实时显示转让回收价
- ✅ 换购自由：转让后可购买更高级洞天

**🎯 系统差异化定位**
- 🚶 历练系统：日常材料农场（灵草、精铁等炼丹材料）
- 🌀 秘境系统：装备功法刷宝副本（装备、功法、稀有材料）
- 形成互补：历练稳定产出 + 秘境爆率玩法

---

## 📄 许可证

本项目采用 [AGPL-3.0](LICENSE) 许可证。
